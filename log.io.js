...
jQuery.fn.extend({
	...
	closest: function( selectors, context ) {
		var cur,
			...
			l = this.length,
			ret = [],
			pos = rneedsContext.test( selectors ) || typeof selectors !== "string" ?  jQuery( selectors, context || this.context ) :
				0;
...
require.define("/src/templates.coffee",function(require,module,exports,__dirname,__filename,process,global){(function() {
  ...
  exports.logMessage = "<p>\n<span class='stream color<%= lmessage.get('stream').color %>'><%= lmessage.get('stream').id %></span>\n<span class='node color<%= lmessage.get('node').color %>'><%= lmessage.get('node').id %></span>\n<span class='message' <%\nvar mapping = {'Verbose':'#A8A8A8','Debug':'#81A2BE','Info':'#EEE','Warning':'#FFFB00','Error':'#CC6666','Severe':'#F00'}\nfor(var k in mapping) {\nif(msg.startsWith('['+k+']')){ %>style='color:<%= mapping[k] %>' <%}\n}\n%>><%= msg %></span>\n</p>";
